...
GCC 컴파일러가 정상적으로 설치되어 있으며, C 프로그램을 컴파일할 수 있는 환경이 준비된 것을 확인하였다.
...
char 타입은 -128부터 127까지의 값을 저장할 수 있다. 가장 작은 값인 -128에서 1을 빼면 저장 가능한 범위를 벗어나므로 127로 변경된다. 이러한 현상을 **언더플로(Underflow)**라고 한다.
...
- `&`(AND) 연산을 수행하여 원하는 위치의 비트만 0으로 바꾸고 나머지 비트는 그대로 유지한다.
...
기존에는 값이 코드에 고정되어 있었는데, 사용자가 직접 값과 비트 위치를 입력하도록 수정하였다. 입력값을 바꿔가며 비트 연산 결과를 확인할 수 있었다.
...
### 실행 결과
컴파일을 진행하면서 전처리, 컴파일, 어셈블, 링킹 단계를 거쳐 최종 실행 파일이 생성되는 과정을 확인할 수 있었으며, 단계별로 생성되는 파일의 종류와 크기가 달라지는 것도 확인하였다.

### 파일 크기 비교

컴파일 단계가 진행되면서 생성되는 파일의 종류와 크기가 달라지는 것을 확인하였다.
